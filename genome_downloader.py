...
#                        for file_id in fil:
#                        print file_id
                        print "Downloading %s from %s via urrlib at %s" % (fil, pwd + "/" + str(key) + "/" + str(annoying_folders[key]) + "/" + str(genome_subfolders[key[genome_subfolders.index(key)]]), datetime.datetime.now())
